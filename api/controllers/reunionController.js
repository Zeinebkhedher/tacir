...
const getReunionsByProjectHolderId = async (req, res) => {
  try {
    const projectHolderId = req.params.projectHolderId;
    
    // Fetch reunions where the project holder ID is in the destinataires array
    const reunions = await Reunion.find({
      destinataires: projectHolderId,
    });
