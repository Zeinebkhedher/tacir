...
const BesoinAccomapgnement = () => {
    ...
    return (
      <div style={{marginBottom:"10%"}}>
        <DialogTitle style={{fontSize:"35px"}}>Besoins d'accompagnement</DialogTitle>
              ...
              {besoinsMateriels.map((besoin, index) => (
                <>
                  <div key={index} style={{ display: 'flex'}}>
                    <p>{besoin}</p>
                    <Button onClick={() => handleDelete('Ressources matérielles', index)} style={{marginLeft:"60%" ,borderRadius:"30px", backgroundColor:"transparent"}}>
            ...
            <Paper elevation={3} style={{ borderRadius: '20px', backgroundColor: '#81D4FA', padding: '20px', width:"130%", maxHeight: '500px', overflow: 'auto' }}>
              ...
              {besoinsFormation.map((besoin, index) => (
                <>
                  <div key={index} style={{ display: 'flex'}}>
                    <p>{besoin}</p>
                    <Button onClick={() => handleDelete('Besoins de formation', index)} style={{marginLeft:"50%" ,borderRadius:"30px", backgroundColor:"transparent"}}>
                      <MdDelete style={{fontSize:"20px"}} />
                    </Button>
                  </div>
                  <hr />
