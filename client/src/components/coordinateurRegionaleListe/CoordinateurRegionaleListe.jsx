...
const CoordinateurRegionaleListe = () => {
  ...
  const fetchCandidatures = async () => {
    try {
      const res = await axios.get("http://localhost:8000/api/membres/getAllMembers?role=coordinateurRegional");
  ...
  const actionColumn = [
    {
      ...
      renderCell: (params) => {
        return (
          <div className="cellActiondash" style={{ display: "flex", justifyContent: "center", backgroundColor:"#D2B4DE" }}>
            <Button
              variant="outlined"
              size="small"
    ...
    <>
      ...
      {selectedCandidature && (
        <Dialog open={open} onClose={handleClose} aria-labelledby="candidature-dialog-title">
          <DialogTitle id="candidature-dialog-title">Détails de la Candid   ature</DialogTitle>
          <DialogContent>
            <DialogContentText>
...
export default CoordinateurRegionaleListe;
